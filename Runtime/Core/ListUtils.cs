...
namespace HP.Framework.Common
{
    public static class ListUtils
    {
        ...
        public static List<TOut> Map<TIn, TOut>(IList<TIn> list, MapFunc<TOut, TIn> func)
        {
            List<TOut> newList = new List<TOut>(list == null ? 0 : list.Count);
            Map(list, newList, func);
            return newList;
        }

        // Maps a list into a pre-allocated results list (Zero GC).
        public static void Map<TIn, TOut>(IList<TIn> list, IList<TOut> results, MapFunc<TOut, TIn> func)
        {
            ...
            {
                results.Add(func(list[i]));
            }
        }

        // Filters a list into a new list.
        public static List<T> Filter<T>(IList<T> list, FilterFunc<T> func)
        {
            List<T> newList = new List<T>();
            Filter(list, newList, func);
            return newList;
        }

        // Filters a list into a pre-allocated results list (Zero GC).
        public static void Filter<T>(IList<T> list, IList<T> results, FilterFunc<T> func)
        {
            if (results == null) return;
            results.Clear();
