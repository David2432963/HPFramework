...
namespace HP.Framework.Diagnostics
{
    public sealed class DraggableSnapPanel : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IInitializePotentialDragHandler
    {
        ...
        private void Awake()
        {
            ...
            if (dragTarget == null)
            {
                Debug.LogWarning($"{nameof(DraggableSnapPanel)} on {name} requires a RectTransform.", this);
                enabled = false;
                return;
            }
        ...
        public void OnBeginDrag(PointerEventData eventData)
        {
            ...
            if (!TryGetParentRect(out RectTransform parent)
                || !TryGetPointerWorldPoint(parent, eventData, out Vector3 worldPoint))
            {
                return;
            }
        ...
        {
            if (!isDragging
                || !TryGetParentRect(out RectTransform parent)
                || !TryGetPointerWorldPoint(parent, eventData, out Vector3 worldPoint))
            {
                return;
            }
        ...
        private void GetCorners(RectTransform parent)
        {
        ...
        {
            return max < min ? (min + max) * 0.5f : Mathf.Clamp(value, min, max);
        }
        ...
        private void StartSnapAnimation(Vector3 targetPosition)
        {
            ...
            {
                snapRoutine = StartCoroutine(SnapToRoutine(targetPosition));
            }
            else
            {
        ...
        private System.Collections.IEnumerator SnapToRoutine(Vector3 targetPosition)
        {
